...
export function exportToTtl(state: ModelBuilderState): string {
  ...
  lines.push(`@prefix gdb: <https://graphdbnext.example.com/schema/> .`)
  lines.push('')
...
export function exportToRdf(state: ModelBuilderState): string {
  ...
  lines.push(`  xmlns:gdb="https://graphdbnext.example.com/schema/">`)
  lines.push('')
  ...
  state.nodes.forEach(node => {
    ...
    lines.push(`  <rdfs:Class rdf:about="https://graphdbnext.example.com/schema/${classId}">`)
    ...
    if (node.type !== node.label) {
      lines.push(`    <rdfs:subClassOf rdf:resource="https://graphdbnext.example.com/schema/${normalizeId(node.type)}" />`)
    }
  ...
  relationsByType.forEach((relData, relType) => {
    const relId = normalizeId(relType)
    lines.push(`  <rdf:Property rdf:about="https://graphdbnext.example.com/schema/${relId}">`)
    ...
    allDomains.forEach(domain => {
      lines.push(`    <rdfs:domain rdf:resource="https://graphdbnext.example.com/schema/${normalizeId(domain)}" />`)
    })
    
    allRanges.forEach(range => {
      lines.push(`    <rdfs:range rdf:resource="https://graphdbnext.example.com/schema/${normalizeId(range)}" />`)
    })
  ...
  state.nodes.forEach(node => {
    node.properties.forEach(prop => {
      ...
      lines.push(`  <rdf:Property rdf:about="https://graphdbnext.example.com/schema/${normalizeId(node.label)}_${propId}">`)
      ...
      lines.push(`    <rdfs:domain rdf:resource="https://graphdbnext.example.com/schema/${normalizeId(node.label)}" />`)
  ...
  relationsByType.forEach((relData, relType) => {
    if (relData.properties) {
      relData.properties.forEach(prop => {
        const propId = normalizeId(prop.key)
        lines.push(`  <rdf:Property rdf:about="https://graphdbnext.example.com/schema/${normalizeId(relType)}_${propId}">`)
        lines.push(`    <rdfs:label>${prop.key}</rdfs:label>`)
        lines.push(`    <rdfs:range rdf:resource="${mapPropertyTypeToXSDFull(prop.type)}" />`)
        lines.push(`  </rdf:Property>`)
        lines.push('')
...
export function exportDataToTtl(state: ModelBuilderState, graphItems: Record<string, unknown>[]): string {
  ...
  const docUrl = `https://graphdbnext.example.com/data/${docName}/`
  prefixMap.set(docPrefix, docUrl)
  prefixMap.set('gdb', 'https://graphdbnext.example.com/schema/')
