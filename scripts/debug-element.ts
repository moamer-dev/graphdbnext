...
if (target) {
  console.log('Tag:', converter['elementHelper'].getTag(target));
  console.log('Text:', JSON.stringify(target.text));
  console.log('Children count:', target.children.length);
  target.children.forEach((child: any, idx: number) => {
    console.log(` Child ${idx} tag:`, converter['elementHelper'].getTag(child));
