...
Open `packages/plexus-builder/src/registry/actions/definitions.ts` and use the `registerAction` helper. This single call now handles both the palette appearance and the sidebar configuration.
...
Open `packages/plexus-builder/src/registry/tools/definitions.ts` and use the `registerTool` helper.
...
Add the new type to the `FieldType` union in `packages/plexus-builder/src/registry/types.ts`.
...
Add a new `case` block in the `renderField` function within `packages/plexus-builder/src/components/shared/SchemaForm.tsx`. Use standard UI components and call the `onChange` prop to update the state.
