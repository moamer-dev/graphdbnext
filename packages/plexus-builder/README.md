...
The `plexus-builder` package is the core engine of the Plexus model designer. It provides a visual interface for:
...
This package is designed for the Plexus ecosystem but is fully portable and can be used in any React application.
...
import type { AISettings, WorkflowPersistence } from '@plexus/builder';
...
```tsx
import { ModelBuilder, AISettingsProvider } from '@plexus/builder';
import '@plexus/builder/dist/index.css';
...
The `plexus-builder` features several AI-driven agents. Configuration is handled via the `AISettingsProvider`. For persistent, user-specific configurations, it is recommended to fetch these settings from your application's database via an API endpoint.
...
module.exports = {
  content: [
    "./src/**/*.{js,ts,jsx,tsx}",
    "./node_modules/@plexus/builder/src/**/*.{js,ts,jsx,tsx}",
  ],
}
```
